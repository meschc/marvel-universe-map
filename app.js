...
const PHASE_COLORS = {"1": "#e23636", "2": "#f2a900", "3": "#3ecf8e", "4": "#4fc3f7", "5": "#7e57c2", "6": "#ff4d6d", "7": "#ffd54f", "netflix": "#8b0000", "other": "#888", "xmen": "#2979ff", "raimi": "#ff7043", "webb": "#26c6da", "sony": "#b0bec5", "animation": "#00bfa5", "spiderverse": "#d500f9"};
const PHASE_LABELS_RU = {"1": "Фаза 1", "2": "Фаза 2", "3": "Фаза 3", "4": "Фаза 4", "5": "Фаза 5", "6": "Фаза 6", "7": "Фаза 7", "netflix": "Netflix", "other": "Другое", "xmen": "Люди Икс (Fox)", "raimi": "Человек-паук (Рэйми)", "webb": "Нов. Человек-паук (Уэбб)", "sony": "Вселенная Sony", "animation": "Мультфильмы", "spiderverse": "Spider-Verse (Sony)"}; const PHASE_LABELS_EN = {"1": "Phase 1", "2": "Phase 2", "3": "Phase 3", "4": "Phase 4", "5": "Phase 5", "6": "Phase 6", "7": "Phase 7", "netflix": "Netflix", "other": "Other", "xmen": "X-Men (Fox)", "raimi": "Spider-Man (Raimi)", "webb": "Amazing Spider-Man (Webb)", "sony": "Sony's Universe", "animation": "Animation", "spiderverse": "Spider-Verse (Sony)"};
...
// ---- edge-label translations (v6.5): connection labels used to be stored RU-only
// (e.g. "лучший друг", "Красная комната") or as raw snake_case event codes
...
(function injectPhase7Stories(){
  ...
  const charIds = new Set(DATA.characters.nodes.map(c=>c.id));
  const NS = [
    {"id":"story_the_punisher_one_last_kill","title":"The Punisher: One Last Kill","title_ru":"Каратель: Последнее убийство","type":"tv_series","date":"2026-05-12","phase":"6","characters":["punisher","karen_page"],"char_count":2,"poster":"https://static.wikia.nocookie.net/marvelcinematicuniverse/images/1/1b/The_Punisher_One_Last_Kill_Poster.jpg/revision/latest?cb=20260513022539","event_year":2026.5,"event_date_ru":"2026","event_date_en":"2026","universe":"Earth-199999"},
    ...
    {"id":"story_ghost_rider_2028","title":"Ghost Rider","title_ru":"Призрачный гонщик","type":"movie","date":"2028-07-28","phase":"7","characters":[],"char_count":0,"poster":"https://static.wikia.nocookie.net/marvelcinematicuniverse/images/1/13/Ghost_Rider_SDCC_Logo.jpg/revision/latest?cb=20260726011130","event_year":2028.7,"event_date_ru":"TBA (ожидается)","event_date_en":"TBA (upcoming)","universe":"Earth-199999"},
    {"id":"story_black_panther_iii","title":"Black Panther III","title_ru":"Чёрная пантера 3","type":"movie","date":"2028-12-15","phase":"7","characters":["black_panther","m_baku"],"char_count":2,"poster":"https://static.wikia.nocookie.net/marvelcinematicuniverse/images/7/70/Black_Panther_III_Logo.jpg/revision/latest?cb=20260726013922","event_year":2028.95,"event_date_ru":"TBA (ожидается)","event_date_en":"TBA (upcoming)","universe":"Earth-199999"}
  ];
  NS.forEach(s=>{
    s.characters = (s.characters||[]).filter(cid=>charIds.has(cid));
    s.char_count = s.characters.length;
...
function showCharDetail(d){
  ...
  if (rel.length) {
    ...
    for (const l of rel) {
      ...
      h += `<div class="rel-item" data-goto="${other.id}"><span>${oname}</span><span class="lbl" style="color:${EDGE_COLORS[l.type]}">${EDGE_LABELS()[l.type]}${l.label?(' · '+translateEdgeLabel(l.label, LANG)):''}</span></div>`;
    }
  }
...
function refreshStats(){
  ...
  if (!statsEl) return;
  // Universe count is shown on EVERY tab (user asked for it), using the same source of
  // truth as the characters tab (distinct universes across character nodes = 12). This
  // matches the title/SEO/og-image "12 universes" everywhere and stays consistent even
  // for comics (whose nodes carry no universe field of their own).
  const uniCount = new Set(charNodes.map(n=>n.universe)).size;
  if (MODE==='characters') statsEl.textContent = charNodes.length+u.stats_chars+uniCount+u.stats_uni+charLinks.length+u.stats_edges;
  else if (MODE==='stories') statsEl.textContent = storyNodes.length+u.stats_stories+uniCount+u.stats_uni+storyLinksRaw.length+u.stats_edges;
  else statsEl.textContent = comicNodes.length+u.stats_comics+uniCount+u.stats_uni+comicLinksRaw.length+u.stats_edges;
}
...
const MSheet = (function(){
  ...
  function init(){
    ...
    document.getElementById('m-search-btn').addEventListener('click', ()=>{
      ...
      // on mobile the search sheet searches EVERYTHING (searchAll covers characters,
      // movies AND comics regardless of the active tab), so the desktop placeholder
      // "Поиск персонажа…" (which is per-mode) is misleading here — use the all-scope one.
      const s = document.getElementById('search');
      if (s) { s.placeholder = UI().search_ph_all; setTimeout(()=>s.focus(), 120); }
    });
