...
function translateEdgeLabel(lbl){ if(!lbl) return lbl; const t = EDGE_LABEL_TR[lbl]; if(!t) return lbl; return (LANG==='ru' ? t.ru : t.en) || lbl; }
...
(function injectMissingStories(){
  ...
  const NC = [
    ...
    { id:'sue_storm_fox', name:'Invisible Woman', name_ru:'Женщина-невидимка', real_name:'Susan Storm', real_name_ru:'Сьюзан Шторm',
...
function showCharDetail(d){
  ...
  if (rel.length) {
    ...
    for (const l of rel) {
      ...
      h += `<div class="rel-item" data-goto="${other.id}"><span>${oname}</span><span class="lbl" style="color:${EDGE_COLORS[l.type]}">${EDGE_LABELS()[l.type]}${l.label?(' · '+translateEdgeLabel(l.label)):''}</span></div>`;
    }
  }
