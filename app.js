...
function computeClusterCenters(){
  ...
  const rad = {};
  UNIVERSE_ORDER.forEach(u=>{ rad[u] = Math.max(120, 60 + Math.sqrt(counts[u]||1)*46); });
  const cx = w/2, cy = h/2;
...
function buildCharGraph(){
  ...
  if (CHAR_LAYOUT === 'universe') {
    ...
    sim.force('link', d3.forceLink(currentLinks).id(d=>d.id).distance(46).strength(l=> l.type==='variant'?0.008:0.15))
       .force('charge', d3.forceManyBody().strength(-140))
       .force('x', d3.forceX(d=> (clusterCenters[d.universe]||clusterCenters._fallback)[0]).strength(0.22))
       .force('y', d3.forceY(d=> (clusterCenters[d.universe]||clusterCenters._fallback)[1]).strength(0.22))
       .force('collision', d3.forceCollide().radius(d=>charRadius(d)+9).strength(0.9));
    // pre-settle so the initial view is already spread out, not a moving blob
    sim.alpha(1); for (let k=0;k<160;k++) sim.tick();
  } else {
    sim.force('link', d3.forceLink(currentLinks).id(d=>d.id).distance(l=>46+(l.type==='team'?10:0)).strength(0.35))
       ...
       .force('collision', d3.forceCollide().radius(d=>charRadius(d)+6));
  }

  const defs = svg.append('defs');
  if (CHAR_LAYOUT === 'universe') {
    // draw rays from the MCU hub out to every other universe (the "star" spokes)
    const hub = clusterCenters[clusterCenters._hub];
    const rayG = g.append('g').attr('class','uni-rays');
    UNIVERSE_ORDER.forEach(u=>{
      if (u === clusterCenters._hub) return;
      const c = clusterCenters[u]; if(!c) return;
      rayG.append('line').attr('x1', hub[0]).attr('y1', hub[1]).attr('x2', c[0]).attr('y2', c[1])
        .attr('stroke', UNIVERSE_COLORS[u]||'#888').attr('stroke-width', 2).attr('stroke-opacity', 0.28);
    });
  ...
  nodeSel.each(function(d){
    ...
    const body = sel.append('circle').attr('r', r).attr('fill', GROUP_COLORS[d.group]||'#999');
    if (d.image) {
      lazyTasks.push(()=>{
        const pid = 'pat_'+d.id;
...
creditsOverlay.addEventListener('click', (ev)=>{ if (ev.target === creditsOverlay || ev.target.classList.contains('cc-close')) creditsOverlay.classList.remove('show'); });
